...
def display_network(A, m = -1, n = -1):
    ...
    #print(buf + m * (sz + buf), buf + n * (sz + buf))
    image = np.ones(shape=(int(buf + m * (sz + buf)), int(buf + n * (sz + buf)))) # Using np.ones to make white border btw digits
    ...
    for i in range(int(m)):
        for j in range(int(n)):
            ...
            # Each column of A is a digit image
            clim = np.max(np.abs(A[:, k]))

            if opt_normalize:
                image[buf + i * (sz + buf):buf + i * (sz + buf) + sz, buf + j * (sz + buf):buf + j * (sz + buf) + sz] = \
                    A[:, k].reshape(sz, sz) / clim  # reshape to original image format ~ 28 x 28
            else:
                image[buf + i * (sz + buf):buf + i * (sz + buf) + sz, buf + j * (sz + buf):buf + j * (sz + buf) + sz] = \
